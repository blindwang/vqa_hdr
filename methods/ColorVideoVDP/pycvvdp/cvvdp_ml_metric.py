...
class cvvdp_ml_saliency(cvvdp_ml):
    ...
    def __init__(self, config_paths=[], device=None, **kwargs):
        ...
        # )
        model_path = os.path.join(path, "cvvdp.ckpt")
        if not os.path.exists(model_path):
             print(f"Warning: Model not found at {model_path}, trying fallback download...")
             try:
                model_path = hf_hub_download(
                    repo_id="gfxdisp/cvvdp_ml",
                    filename="cvvdp_ml_saliency/cvvdp.ckpt"
                ...
                raise RuntimeError(f"Could not find model at {model_path} and download failed.")
